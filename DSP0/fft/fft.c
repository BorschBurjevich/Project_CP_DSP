...
void spectrum_power()
{
    uint32_t n = word03;   // N fft
    Cache_inv ((float *)word01, sizeof(float)*2*n, 0x7fff, TRUE);//вынуть из кеша // word01 - data in
    switch(n)
    {
        case 256:
        {
            DSPF_sp_vecmul((float *)word01,window_256,x_sa, 2*n);//наложение окна блакман
            ...
            DSPF_sp_vecmul(y_sa,ten,(float *) word02, n);//10log10(sp) // word02 - data out
            Cache_wb ((float *)word02, sizeof(float)*2*n, 0x7fff, TRUE);//вынуть из кеша
            break;
        };

        case 512:
        {
            DSPF_sp_vecmul((float *)word01,window_512,x_sa, 2*n);
            DSPF_sp_fftSPxSP(n, x_sa, w_512, y_sa, brev, 2, 0, n);
            divsp_v(y_sa, n_512, y_sa, 2*n);//нормировка на число точек
            butterfly(y_sa,x_sa,n);//бабочка
            spectrum_power_asm(x_sa,y_sa, n);//спектр мощнности
            log10sp_v(y_sa, y_sa, n);//log10(sp)
            DSPF_sp_vecmul(y_sa,ten,(float *) word02, n);//10log10(sp)
            Cache_wb ((float *)word02, sizeof(float)*2*n, 0x7fff, TRUE);//вынуть из кеша
            break;
        };
        ...
        case 1024:
        {
            DSPF_sp_vecmul((float *)word01, window_1024,x_sa, 2*n);
...
void spectrum()
{
    ...
    uint32_t     n = word03;
    Cache_inv ((float *)word01, sizeof(float)*2*n, 0x7fff, TRUE);//вынуть из кеша
    switch(n)
    {
        case 256:
        {
            DSPF_sp_vecmul((float *)word01,window_256,x_sa, 2*n);//наложение окна блакмана
            DSPF_sp_fftSPxSP(n, x_sa, w_256, y_sa, brev, 4, 0, n);//fft
            divsp_v(y_sa, n_256, y_sa, 2*n);//нормировка на число точек
            butterfly(y_sa,(float *)word02, n);//бабочка
            Cache_wb ((float *)word02, sizeof(float)*2*n, 0x7fff, TRUE);//вынуть из кеша
            break;
        };
        ...
        case 1024:
        {
            ...
            butterfly(y_sa,(float *)word02, n);
            Cache_wb ((float *)word02, sizeof(float)*2*n, 0x7fff, TRUE);
            break;
        };
    }
}
