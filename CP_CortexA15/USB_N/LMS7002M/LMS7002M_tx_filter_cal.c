    ...
    int *reg_ptr, const int reg_addr, const int reg_max, const char *reg_name)
{
    ...
    while (true)
    {
        ...
        {
            printf("failed to converge when calibrating %s", reg_name);
            return -1;
        }
        ...
        *reg_ptr -= adjust*5;
        ...
        {
            printf("failed to cal %s -> %d", reg_name, *reg_ptr);
            return -1;
        }
    ...
    printf("%s = %d", reg_name, *reg_ptr);
    printf("ccal_lpflad_tbb = %d", LMS7002M_regs(self)->reg_0x010a_ccal_lpflad_tbb);
    return 0;
}
...
static int tx_cal_tbb_lpfs5(LMS7002M_t *self, const LMS7002M_chan_t channel, const double bw)
{
    ...
    {
        printf("LPFS5 bandwidth not in range[0.8 to 3.2 MHz]");
        return -1;
    }
...
static int tx_cal_tbb_lpflad(LMS7002M_t *self, const LMS7002M_chan_t channel, const double bw)
{
    ...
    if (bw < 2e6 || bw > 16e6)
    {
        printf( "LPFLAD bandwidth not in range[2 to 16 MHz]");
        status = -1;
        goto done;
    }
...
static int tx_cal_tbb_lpfh(LMS7002M_t *self, const LMS7002M_chan_t channel, const double bw)
{
    ...
    if (bw < 28e6 || bw > 60e6)
    {
        printf( "LPFH bandwidth not in range[28 to 60 MHz]");
        status = -1;
        goto done;
    }
