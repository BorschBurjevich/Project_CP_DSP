...
#include "USB_N/LMS7002M/LMS7002M_filter_cal.h"
#include "USB_N/LMS7002M/LMS7002M_logger.h"
#include "USB_N/LMS7002M/LMS7002M_time.h"
#include <string.h> //memcpy

#define mask 0x0FFF
#define mask_negative 0xF800
#define bit_12_ADC 1<<11
#define EMIF2 *(uint16_t*)0x38000000
extern uint16_t  DATA_READ[];
extern double GetRate (LMS7002M_t *self, bool tx, double rf_rate_Hz);
extern int SetTestSignal(LMS7002M_t *self, bool dir_tx, unsigned chan, lms_testsig_t sig, int16_t dc_i, int16_t dc_q);
extern void Set_NCO_Freq (LMS7002M_t *self, float Freq_MHz, float refClk_MHz, bool Rx);
/***********************************************************************
 * Re-tune the RX LO based on the bandwidth
 **********************************************************************/
int setup_rx_cal_tone(LMS7002M_t *self, const LMS7002M_chan_t channel, const double bw)
{
    LMS7002M_sxx_enable(self, LMS_RX, true);
    LMS7002M_sxt_to_sxr(self, true);
    int status = 0;
    ...
    if (status != 0)
    {
        printf( "LMS7002M_set_lo_freq(LMS_RX, %f MHz)", sxr_freq/1e6);
        goto done;
    }
    double rxtsp_rate = self->cgen_freq/4;
    const double rx_nco_freq = (self->sxt_freq-sxr_freq_actual)-1e6;//???????????????const double rx_nco_freq = (self->sxt_freq-sxr_freq_actual)-1e6;
...
/*    LMS7002M_regs_spi_read(self, 0x040c);
    LMS7002M_regs_spi_read(self, 0x040a);
    LMS7002M_spi_write(self, 0x040A, 0x3007);
    LMS7002M_regs_spi_read(self, 0x0400);
    LMS7002M_regs_spi_read(self, 0x040e);
    LMS7002M_regs_spi_read(self, 0x040f);
    LMS7002M_regs_spi_read(self, 0x040a);


    LMS7002M_spi_write(self,0x0400, 0x0081);
    delay_Ticks(1000);
    LMS7002M_spi_write(self,0x0400, 0x8081);
*/
    //--- gain selection ---//
    ...
    while (true)
    {
        ...
        {
            printf( "failed to converge when calibrating %s\n", reg_name);
            return -1;
        }
        ...
        *reg_ptr += adjust;
        ...
        uint16_t rssi_test = rssi_value_50k*0.7071;
        rssi_test = rssi_test && adjust;

        if (rssi_value < rssi_value_50k*0.7071 && adjust > 0) break;
        if (*reg_ptr == 0 || *reg_ptr == reg_max)
        {
            printf("failed to cal %s -> %d\n", reg_name, *reg_ptr);
            return -1;
        }
    }
    printf("%s = %d\n", reg_name, *reg_ptr);
...
static int rx_cal_init(LMS7002M_t *self, const LMS7002M_chan_t channel)
{
    ...
    //--- sxt ---
    const double sxt_freq = 104.3e6/*500e6*/;
    status = LMS7002M_set_lo_freq(self, LMS_TX, self->sxt_fref, sxt_freq, NULL);
    LMS7002M_set_mac_ch(self, channel);
    if (status != 0)
    {
        printf("LMS7002M_set_lo_freq(LMS_TX, %f MHz)", sxt_freq/1e6);
        goto done;
    }
    ...
    LMS7002M_txtsp_tsg_const(self, channel, 0x7fff, 0x8000);//0x7fff , 0x8000
    ...
    //--- initial cal tone ---//
    status = setup_rx_cal_tone(self, channel, 0.150e6);
...
static int rx_cal_tia_rfe(LMS7002M_t *self, const LMS7002M_chan_t channel, const double bw)
{
    ...
    if (bw < 0.4e6 || bw > 60e6)
    {
        printf( "TIA bandwidth not in range[0.4 to 60 MHz]");
        status = -1;
        goto done;
    }
    ...
    {
        printf("g_tia_rfe must be [1, 2, or 3], got %d", g_tia_rfe_user);
        status = -1;
        goto done;
    }
...
static int rx_cal_rbb_lpfl(LMS7002M_t *self, const LMS7002M_chan_t channel, const double bw)
{
    ...
    if (bw < 0.4e6 || bw > 20e6)
    {
        printf("LPFL bandwidth not in range[0.4 to 20 MHz]");
        status = -1;
        goto done;
    }
...
static int rx_cal_rbb_lpfh(LMS7002M_t *self, const LMS7002M_chan_t channel, const double bw)
{
    ...
    if (bw < 20e6 || bw > 60e6)
    {
        printf("LPFH bandwidth not in range[0.5 to 60 MHz]");
        return -1;
    }
...
int LMS7002M_rbb_set_filter_bw(LMS7002M_t *self, const LMS7002M_chan_t channel, double bw, double *bwactual)
{
    ...
    if (status != 0)
    {
        printf("cal_setup_cgen() faled\n");
        goto done;
    }
    double sampleRate = GetRate (self, 0, 20.000e6);
