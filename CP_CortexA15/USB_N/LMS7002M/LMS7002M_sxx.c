...
void LMS7002M_sxx_apply_tune_state(LMS7002M_t *self, const LMS7002M_sxx_tune_state *s)
{
    ...
    //printf("fdiv = %d, Ndiv = %f, Nint = %d, Nfrac = %d, DIV_LOCH_SX = %d, fvco = %f MHz\n", s->fdiv, s->Ndiv, s->Nint, s->Nfrac, s->DIV_LOCH_SX, s->fvco/1e6);
...
int LMS7002M_set_lo_freq(LMS7002M_t *self, const LMS7002M_dir_t direction, const double fref, const double fout, double *factual)
{
    ...
    for (int SEL_VCO_i = 0; SEL_VCO_i < 4; SEL_VCO_i++)
    {
    ...
    if (SEL_VCO_best == -1)
    {
        printf("VCO select FAIL\n");
        return -3;
    }

    printf( "Choosing SEL_VCO = %d\n", SEL_VCO_best);
