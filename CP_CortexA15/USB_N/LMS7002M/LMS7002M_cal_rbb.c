...
void RBB_Algorithm_A(LMS7002M_t *self)
{
    ...
    Modify_SPI_Reg_bits (self, 0x0116, 15, 11, 16);
...
unsigned char RBB_Algorithm_B(LMS7002M_t *self)
{
    ...
    Set_NCO_Freq(self, 0.1, self->cgen_freq/1000, true);
 ...
 * RBB Algorithm F
 **********************************************************************/
void RBB_Algorithm_F(LMS7002M_t *self, unsigned char Band_id)
{
...
//	Set_NCO_Freq (RBB_CalFreq[Band_id], fclk, false); // Apply a single tone frequency at CalFreq.
	...
	while (1)
	{
		...
		ADCOUT = (Get_SPI_Reg_bits(self, 0x040F, 15, 0) << 2) | Get_SPI_Reg_bits(self, 0x040E, 1, 0); //RSSI value; //RSSI value // Measure the value of the amplitude at the ADC input. This value should be lower than LowFreqAmp.
