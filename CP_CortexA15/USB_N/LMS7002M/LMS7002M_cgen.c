...
int LMS7002M_set_data_clock(LMS7002M_t *self, const double fref, const double fout, double *factual)// factual -частота дискретизации
{
    //printf("CGEN tune %f MHz (fref=%f MHz) begin \n", fout/1e6, fref/1e6);
    uint32_t answer_dat = 0;
    ...
    while (true)
    {
        ...
        Ndiv = fout*fdiv/fref;
        fvco = fout*fdiv;
       // printf( "Trying: fdiv = %d, Ndiv = %f, fvco = %f MHz \n", fdiv, Ndiv, fvco/1e6);
        ...
        //if (fvco < LMS7002M_SXX_VCOH_LO) continue;
        //if (fvco > LMS7002M_SXX_VCOH_HI) continue;

        break; //its good
    }
        //printf("Using: fdiv = %d, Ndiv = %f, fvco = %f MHz \n", fdiv, Ndiv, fvco/1e6);
    ...
    LMS7002M_regs_spi_write(self, 0x0086);
    answer_dat = LMS7002M_regs_spi_read(self, 0x0086);
    ...
    answer_dat = LMS7002M_regs_spi_read(self, 0x0087);
    LMS7002M_regs_spi_write(self, 0x0088);
    answer_dat = LMS7002M_regs_spi_read(self, 0x0088);
    ...
    LMS7002M_usb_write(0x008a,0x0514);  // добавил сам
    ...
    {
        LMS7_logf( LMS7_DEBUG, "VCO select FAIL \n");
        return -3;
    }
