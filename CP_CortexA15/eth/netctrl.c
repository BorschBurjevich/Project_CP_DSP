                 ...
                 void (*NetStop)(), void (*NetIP)(uint32_t,uint32_t,uint32_t) )
{
    ...
    /*HW_WR_REG32(0x48484D84, 0x000000A1);//изменение на 1 гбит/c
    HW_WR_REG32(0x48484DC4, 0x000000A1);//изменение на 1 гбит/c*/

    HW_WR_REG32(0x48484D84, 0x00000021);//изменение на 100 мбит/c
    HW_WR_REG32(0x48484DC4, 0x00000021);//изменение на 100 мбит/c
...
{
    NetReturnCode = rc;//0 = выключение стека, 1 = перезагрузка стека, -1 = фатальна ошибка
    NetHaltFlag   = 0;///////////////////////////////grosh изменение на 0 что бы не вылетал ethernet
}
...
static void NetScheduler( uint32_t const SerialCnt, uint32_t const EtherCnt )
{
    ...
    while( !NetHaltFlag )
    {
        if( stkEvent.hSemEvent )
        {
            SemPendBinary( stkEvent.hSemEvent, SEM_FOREVER );/////////////////////////////////////////////////////1
        }
        ...
        {
            stkEvent.EventCodes[STKEVENT_TIMER] = 0;/////////////////////////////////////////////////////////////1
        ...
        {
            NIMUPacketServiceCheck (fEvents);/////////////////////////////////////////////////////////////////////1
